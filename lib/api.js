...
async function handleScheduleTrigger(context) {
  ...
  logger.debug(`Listing issues in testing column number: ${projectColumnNumber} ...`);
  const { data: testingCards } = await octokit.projects.listCards({
    column_id: projectTestingColumnNumber,
  });

  if (cards.length === 0 && testingCards.length === 0) {
    logger.info(`Ready to merge column ${projectColumnNumber} and testing column ${projectTestingColumnNumber} don't have any issues. Cancelling automerge job.`);
    return;
  }
  ...
  for (const pullRequest of pullRequestsTesting) {
    try {
      await update(context, pullRequest);
      ++updated;
    } catch (e) {
