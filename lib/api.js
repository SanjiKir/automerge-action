...
async function handleScheduleTrigger(context) {
  const { octokit, config: { projectColumnNumber } } = context; 
  ...
  if (cards.length === 0) {
    logger.info(`Column ${projectColumnNumber} doesn't have any issues. Cancelling automerge job.`);
    return;
  }

  const pullRequestNumbers = cards.reduce((acc, card) => {
