...
async function merge(context, pullRequest, reviews = []) {
  ...
  const { commitTitle, commitMessage } = getCommitMessage(pullRequest);
...
function skipPullRequest(context, pullRequest, reviews) {
  const {
    config: { mergeForks, mergeLabels, mergeAproovedByReviewers, skipLabelCheck }
  } = context;
  ...
  if (mergeAproovedByReviewers.length > 0 && !reviewer) {
      logger.info("Skipping PR merge, didn't find an approve by a codeowner");
      skip = true;
  } else {
    logger.info(`Found approve review by ${reviewer}. PR can be merged.`);
  }
...
function tryMerge(
  ...
  return retry(
    ...
    mergeRetrySleep,
    () =>
      mergePullRequest(octokit, pullRequest, head, mergeMethod, commitMessage, commitTitle),
    async () => {
      const pr = await getPullRequest(octokit, pullRequest);
      if (pr.merged === true) {
        return "success";
...
function getCommitMessage(pullRequest) {
    let commitTitle, commitMessage;
    commitTitle = pullRequest.title;
    const { body } = pullRequest;

    if (body) {
      const match = body.match(/^ISSUES CLOSED:.*/);
      commitMessage = match ? match[0] : undefined;
    }
